...
static guint32 s7comm_decode_ud_tis_param(tvbuff_t *tvb, proto_tree *tree, guint8 type, guint16 tp_size, guint32 offset);
...
static const value_string tis_param2_names[] = {    /* TODO: Werte und Bezeichnungen sind nicht immer eindeutig zuzuordnen */
...
static gint hf_s7comm_varstat_req_memory_area = -1;          /* Memory area, 1 byte, stringlist userdata_prog_vartab_area_names  */
...
s7comm_decode_ud_security_subfunc(tvbuff_t *tvb,
                                  ...
                                  guint32 dlength,            /* length of data part */
                                  guint32 offset)             /* offset on data part */
{
...
s7comm_decode_ud_pbc_pre_reass(tvbuff_t *tvb,
                               ...
                               guint16 *data_len,          /* length of data part */
                               guint32 *r_id,              /* R_ID of the PBC communication */
                               guint32 offset)             /* offset on data part */
{
    if ((type == S7COMM_UD_TYPE_REQ || type == S7COMM_UD_TYPE_RES) && (*data_len >= 8)) {
...
s7comm_decode_message_service(tvbuff_t *tvb,
                              ...
                              guint8 type,                /* Type of data (request/response) */
                              guint32 dlength,            /* length of data part */
                              guint32 offset)             /* oOffset on data part */
{
    guint8 events;
    guint8 almtype;
                                          ...
                                          guint32 offset)             /* Offset on data part +4 */
{
    ...
    /* TODO: Das ist hier etwas anderes, immer 0x00, 0x01 ? */
    proto_tree_add_item(msg_item_tree, hf_s7comm_cpu_alarm_message_function, tvb, offset, 1, ENC_BIG_ENDIAN);
    offset += 1;
                           ...
                           guint32 offset)
{
    ...
    switch (type)
    {
        ...
        case S7COMM_UD_TYPE_PUSH:
            ...
            if (item_count % 2) {
                proto_tree_add_item(td_tree, hf_s7comm_data_fillbyte, tvb, offset, 1, ENC_BIG_ENDIAN);
                offset += 1;
            }
        }
    return offset;
                               ...
                               guint32 offset)
{
    ...
    if (type == S7COMM_UD_TYPE_REQ) {
         ...
         * from the job setup is known. As the STW is only 16 Bits, and all other registers 32 Bits this
                                ...
                                guint32 offset)
{
    switch (type)
    {
        ...
        case S7COMM_UD_TYPE_RES:
        case S7COMM_UD_TYPE_PUSH:
            /* Die beiden Bausteinnummern und Adressen sind wenn innerhalb eines Bausteins beobachtet wird
             * immer identisch. Bei einem Ruecksprung aus einem anderen Baustein, enthaelt die erste Adresse
             * den aktuellen Haltepunkt, die zweite die Adresse von der aus zurueckgesprungen wurde.
             */
            proto_tree_add_item(td_tree, hf_s7comm_tis_interrupted_blocktype, tvb, offset, 2, ENC_BIG_ENDIAN);
            offset += 2;
            proto_tree_add_item(td_tree, hf_s7comm_tis_interrupted_blocknr, tvb, offset, 2, ENC_BIG_ENDIAN);
        ...
        if (len >= 2) {
            ...
            switch (funcgroup) {
                case S7COMM_UD_FUNCGROUP_NCPRG:
                    ...
                    /* Leider ist es bei NC so, dass auch die erste PDU wenn unfragmentiert als reassembled angezeigt wird.
                     * Weil data_unit_ref bei grossen Dateien ueberlaufen kann und wieder bei 0 beginnt.
                     */
            ...
            if (is_fragmented) {            /* fragmented */
                ...
                /* NC programming uses a different method of fragment indication. The sequence number is used as reference-id, the
                 ...
                 */
                //if (funcgroup == S7COMM_UD_FUNCGROUP_NCPRG) {
                    /* fragment_add_seq_check needs a fragment number 0 for the first fragment. In the protocol
                     * the first fragment has 1.
                     */
                    
                //    fd_head = fragment_add_seq_check(&s7comm_reassembly_table,
                //                                    tvb, offset, pinfo,
...
proto_register_s7comm (void)
{
    ...
    static hf_register_info hf[] = {
        ...
        { "Memory area", "s7comm.varstat.req.memory_area", FT_UINT8, BASE_DEC, VALS(userdata_prog_vartab_area_names), 0x0,
          NULL, HFILL }},
